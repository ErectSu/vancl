...
ClickSourceVancl.prototype.get_param = function (d) {
    ...
    var c = f.split("&amp;");
    for (var b = 0; b & lt; c.length; b++) {
        var e = c[b].indexOf("=");
        if (e == -1) { continue } var a = c[b].substring(0, e);
...
ClickSourceVancl.prototype.getHost = function (d) {
    ...
    var c = /.*\:\/\/([^\/]*).*/;
    var b = d.match(c);
    if (typeof b != "undefined" & amp;& amp; b != null) {
        a = b[1]
    }
    return a
};
...
ClickSourceVancl.prototype.excuteFunction = function (a) {
    try {
        if (window.location.host.indexOf("vancl.com") & lt; 0) { return }
        try {
        ...
        if (window.document.referrer & amp;& amp; window.document.referrer.indexOf("soso.com") & gt;=0) {
            VanclUnionClick();
            return
        }
        ...
        if (j != null & amp;& amp; j != "") {
            ...
            if (k == "") {
                ...
                if (g != null & amp;& amp; g != "") { k = "ad-0-1-" + g + "-0" } var l = a.get_param("srcId");
                if (l != null & amp;& amp; l != "") { k = "ad-0-3-0" + l + "-1" }
            ...
            var c = a.clickwwwname + "websource/websource.aspx?source=" + j + "&amp;sourceInfo=" + k + "&amp;referer=" + encodeURIComponent(a.refUrl) + "&amp;hrefurl=" + encodeURIComponent(a.url) + "&amp;isnew=" + d; a.redirect(c)
        }else {
            var h = a.refUrl;
            if (h != null & amp;& amp; h != "") {
                if (a.getHost(h).indexOf("vancl.com") & lt; 0 & amp;& amp; a.getHost(h).indexOf("vanclimg.com") & lt; 0) {
                    var n = a.getCookie("union_visited");
                    var d;
                    if (n == "1") { d = 0 }
                    else { a.setCookie("union_visited", "1", 1); d = 1 }
                    var f = a.clickwwwname + "websource/websourceurl.aspx?SourceUrl=" + encodeURIComponent(h) + "&amp;hrefurl=" + encodeURIComponent(a.url) + "&amp;isnew=" + d; a.redirect(f)
                }
                    else { var m = a.getCookie("WebSourceTemp"); if (m != "") { var i = a.clickwwwname + "websource/websource.aspx"; a.redirect(i) } }
            }
        }
    } catch (b) { }
};
...
function VanclUnionClick() { 
    ...
    var a = document.getElementsByTagName("head")[0]; a.appendChild(d) 
};